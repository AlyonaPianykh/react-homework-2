...
export class Post extends Component {
  ...
  onShowComments = () => {
    ...
    //    т.е. тоглим отображение комментариев +
      if (this.state.commentsLoaded) {
          const newCommentsSectionExpanded = !this.state.commentsSectionExpanded;
          this.setState({commentsSectionExpanded: newCommentsSectionExpanded});
          return;
      }

    // todo иначе, если запроса не было выполняем его, вызвав метод onLoadComments +
      else this.onLoadComments();
  };
  ...
  render() {
    ...
    const {commentsSectionExpanded, isCommentLoading, error, comments, commentsLoaded} = this.state;
      ...
      <div className={`${CN} card `}>
        ...
        {/* todo создать div который будет выводить ошибку из стейта, если она существует (т.е. не пустая строка) +*/}
              ...
              : null}
          {
              ...
              commentsSectionExpanded && isCommentLoading ?
                  <LoadingIndicator/>
